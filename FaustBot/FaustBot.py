...
from FaustBot.Modules import ActivityObserver, IdentNickServObserver, GiveCookieObserver, LoveAndPeaceObserver, \
    ...
    IntroductionObserver, HangmanObserver, DuckObserver, TestObserver, DuckStatsObserver, RutheObserver
...
class FaustBot(object):
    ...
    def _setup(self):
        ...
        self.add_module(TestObserver.TestObserver())
        self.add_module(DuckStatsObserver.DuckStatsObserver())
        self.add_module(RutheObserver.RutheObserver())
    def run(self):
        self._setup()
        running = True
        while running:
